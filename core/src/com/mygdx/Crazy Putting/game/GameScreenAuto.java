...
import com.mygdx.BotAlgorithms.Bot;
import com.mygdx.BotAlgorithms.GreedyBTalg;
import com.mygdx.BotAlgorithms.GreedyBTalgOptimizated;
import com.mygdx.BotAlgorithms.HCBot;
...
public class GameScreenAuto extends InputAdapter implements Screen {
  ...
  public GameScreenAuto(CrazyPuttingGame game, int level, int alg) {
    ...
    switch (this.alg) {
      case (1):
        this.bot = new GreedyBTalgOptimizated(level, this.treesPos);
        break;
      case (2):
        this.bot = new GreedyBTalg(level, this.treesPos);
        break;
      case (3):
        this.bot = new HCBot(level, this.treesPos);
        break;
      case (4):
        System.out.println("Divide and Conquer bella");
        break;
    }
  }
  ...
  public void render(float delta) {
    if (loaded && ready) {
      ...
      if (rollingBall.isStationary() && state == 1) {
        ...
        if (m.getEndPos().dst(tmpPos.x, tmpPos.y) < 0.7) {
          state = 2;
          this.writer.write(String.valueOf(this.timeNedded), "time");
    ...
    this.batch.draw(this.powerBar, Gdx.graphics.getWidth() - 100, 0, 100, Gdx.graphics.getHeight() * (this.power / 100));
    ...
    if (this.rollingBall.isStationary() && m.getEndPos().dst(tmpPos.x, tmpPos.y) > 0.5 && this.threadAlive == false) {
      Thread newThread = new Thread(new Runnable() {
        ...
        public void run() {
          ...
          timeNedded = timeNedded + end;

          System.out.println(end);
          power = shootingInfo[1];
          angle = getRightAngle(shootingInfo[0]);
          rollingBall.setVelocity((shootingInfo[1] / 10) * 2, fromDegreeToRadians(shootingInfo[0]));
          threadAlive = false;
          state = 1;
          score++;
        }
    ...
    String yourPower = "POWER: " + Math.round(this.power) + " %";
  ...
  private void createLevel(int t) {
    ...
    ball.transform.setTranslation(s.x, (float) Terrain.compute(t, s.x, s.y) / k + this.ballDiameter / 2, s.y);
